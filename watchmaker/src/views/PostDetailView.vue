...
const firstInlineImageUrl = computed(() => {
  const body = post.value?.postBody || ''
  const firstMatch = body.match(/<img[^>]+src=["']([^"']+)["']/i)
...
const pageImage = computed(() => {
  if (titleImageUrl.value) {
    return titleImageUrl.value
  }

  if (firstInlineImageUrl.value) {
    return firstInlineImageUrl.value
  }

  if (legacyGalleryImages.value.length > 0) {
    return legacyGalleryImages.value[0].src
  }
...
useHead(
  computed(() => {
    ...
    const description =
      getPostExcerpt(currentPost?.postBody, 170) ||
      'Beautiful craftsmanship and restoration work from the workshop.'
    const url =
...
<template>
  <section class="relative mx-auto my-4 max-w-7xl px-4 py-12 sm:my-22 sm:px-6 lg:px-8">
    ...
    <div v-if="isLoading" class="mt-8 space-y-8">
      ...
      <div class="bg-primary dark:bg-sec/80 border-acc/20 overflow-hidden rounded-lg border shadow-xl">
      ...
      <div class="mx-auto grid max-w-6xl gap-10"
        ...
        <div class="min-w-0 space-y-10">
          <div class="mx-auto max-w-5xl overflow-hidden rounded-lg">
            <button v-if="titleImageUrl" type="button" class="block w-full cursor-zoom-in"
              @click="openLightboxBySrc(titleImageUrl)">
              <img :src="titleImageUrl" :alt="post.postTitle || 'Featured image'"
          ...
          <header class="mx-auto max-w-3xl space-y-5">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div
                class="text-fg/72 dark:text-fg/82 inline-flex items-center gap-2 text-sm tracking-[0.15em] uppercase">
          ...
          <section v-if="contentHeadings.length" class="mx-auto max-w-3xl lg:hidden">
            <button type="button"
              ...
              <span class="text-fg/60 dark:text-fg/72 text-xs tracking-[0.15em] uppercase">
                {{ contentsOpen ? 'Hide' : 'Show' }}
              </span>
            </button>

            <div v-if="contentsOpen"
              ...
              <nav aria-label="Post contents">
                <ul class="space-y-2">
                  <li v-for="heading in contentHeadings" :key="heading.id">
                    <button type="button"
                      class="text-fg/78 hover:text-acc dark:text-fg/86 flex w-full text-left text-sm leading-relaxed transition-colors"
                      :class="getTocIndentClass(heading.level)" @click="scrollToHeading(heading.id)">
                      {{ heading.text }}
                    </button>
                  </li>
                </ul>
          ...
          <div class="mx-auto max-w-3xl" @click="handleBodyImageClick">
            <PostRichContent :content="post.postBody" empty-message="This post does not have written content yet." />
          </div>
          ...
          <section v-if="legacyGalleryImages.length" class="mx-auto w-full max-w-3xl space-y-5">
            <div class="space-y-2">
              ...
              <p class="text-fg/68 dark:text-fg/78 leading-relaxed">
                This post has additional attached images outside the main story body.
              </p>
            </div>

            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <button v-for="(image, index) in legacyGalleryImages" :key="image.id" type="button"
                ...
                <div class="px-4 py-3">
                  ...
                  <p class="text-fg/60 dark:text-fg/72 text-sm">Click to enlarge</p>
                </div>
              </button>
            </div>
          </section>
        ...
        <aside v-if="contentHeadings.length" class="hidden lg:block">
          <div class="sticky top-28 space-y-4">
            <div class="border-brdr dark:border-sec-mute bg-sec-light/55 dark:bg-sec/76 rounded-lg border p-4">
    ...
    <Teleport to="body">
      <div v-if="activeLightboxImage" class="fixed inset-0 z-[120] flex items-center justify-center bg-black/90 p-4"
        ...
        <div class="absolute top-5 left-5 flex items-center gap-2">
          <button type="button"
            ...
            :disabled="lightboxZoom <= 1" @click="adjustLightboxZoom(-0.25)">
          ...
          <button type="button"
            class="flex size-11 items-center justify-center rounded-full bg-white/10 text-white backdrop-blur-sm transition-colors hover:bg-white/20 disabled:opacity-40"
            :disabled="lightboxZoom >= 4" @click="adjustLightboxZoom(0.25)">
        ...
        <button v-if="lightboxImages.length > 1" type="button"
          ...
          :disabled="lightboxIndex === 0" @click="showPreviousImage">
          <ChevronLeftIcon class="size-6" />
        </button>

        <div class="max-h-[90vh] max-w-6xl overflow-auto rounded-lg" @wheel.prevent="handleLightboxWheel">
          <img :src="activeLightboxImage.src" :alt="activeLightboxImage.alt"
            class="mx-auto block cursor-zoom-in object-contain transition-[width] duration-200" :style="{
        ...
        <button v-if="lightboxImages.length > 1" type="button"
          ...
          :disabled="lightboxIndex === lightboxImages.length - 1" @click="showNextImage">
