...
const handleOpenPost = (post) => {
  // openPost(post) - do not need this - route is the source of truth - see myworkview
...
// Removing due to the duplication in mywork view 
// onMounted(() => {
//   initialize()
// })
</script>
...
<template>
  ...
  <section class="mx-auto max-w-7xl px-2" ref="postsRef">
    ...
    <div v-else class="grid grid-cols-1 gap-6 px-6 sm:grid-cols-2 sm:gap-8 md:gap-10 lg:grid-cols-3 xl:grid-cols-4">
      <article v-for="(post, i) in allPosts.posts" :key="post.postId || i"
        ...
        <div class="relative aspect-[4/3] overflow-hidden">
          <img :src="`/public${post.titleImage.titlePath}` || '/assets/pictures/placeholder.webp'" :alt="post.title"
        ...
        <div v-if="isAuthenticated"
          ...
          <button @click.stop="initEdit(post)" type="button"
    ...
    <!-- Pagination -->
    <PaginationMain :page-numbers="pageNumbers" :pagination-data="allPosts.pagination"
      @page-change="handlePageChangeWithScroll" />

    <!-- Post Modal -->
    <PostModal :show="showPostModal" :post="selectedPost" @close="handleClosePost" @share="handlePostShare" />
